...
describe("terrain projection of engine paths", () => {
  it.each(["vhf-clear", "vhf-ridge", "vhf-ridge-height"])("aligns %s paths with antenna tips", (id) => {
    ...
    expect(points.every((p) => projection.point(p)?.every(Number.isFinite))).toBe(true);
  });
